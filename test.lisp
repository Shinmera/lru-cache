...
(define-test lru-cache
  ...
  (dotimes (i 10)
    (let* ((size (1+ (random 1000)))
           (cache (make-lru-cache size)))
      (dotimes (i (random 10000))
        (if (= 0 (random 2))
            (lru-cache-push (random size) cache)
            (lru-cache-pop (random size) cache)))
      (finish (org.shirakumo.lru-cache::check-lru-cache cache))
